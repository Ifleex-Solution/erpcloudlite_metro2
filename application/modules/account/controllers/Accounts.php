...
class Accounts extends MX_Controller
{
  ...
  public function save_paymentmethod_from_csv()
  {
      ...
      $allowed_natures = ['Cash Nature', 'Bank Nature'];
      if (empty($name))   { echo json_encode(['status' => 'Error', 'message' => 'Payment method name is required']); return; }
      if (!in_array($nature, $allowed_natures)) { echo json_encode(['status' => 'Error', 'message' => 'Invalid nature: ' . $nature]); return; }
      ...
      if ($this->accounts_model->create_method2(['name' => $name, 'nature' => $nature, 'status' => $status])) {
  ...
  public function save_voucher()
  {
    ...
    $num = $this->number_generatorvoucher($this->input->post('type', TRUE),$this->input->post('type2', TRUE));
     ...
     ";
         ...
         echo $this->_build_voucher_print($inserted_id);
  ...
  public function update_voucher()
  {
    ...
    echo $this->_build_voucher_print($this->input->post('id', TRUE));
  }
...
";
  ...
  public function pos_print_voucher()
  {
    echo $this->_build_voucher_print($this->input->post('id', TRUE));
  }

  private function _build_voucher_print($id)
  {
    $encryption_key = Config::$encryption_key;
    ...
    ")
    ->from('voucher v')
    ->join('payment_type p', 'p.id = v.`from`', 'left')
    ->where('v.id', $id)
    ->get()->row_array();

    if (empty($header)) return json_encode(['details' => '']);
    ...
    if ($type == 1 || $type == 2) {
      ...
      ->join('payment_receipt_type prt', 'prt.id = vd.`to`', 'left')
      ->where('vd.pid', $id)
      ->get()->result_array();
    } else {
      $items = $this->db->select("
      ...
      ")
      ->from('voucher_details vd')
      ->join('payment_type pt', 'pt.id = vd.`to`', 'left')
      ->where('vd.pid', $id)
      ->get()->result_array();
    }

    if ($type == 1)      $voucher_no = 'PV-' . $header['voucher_id'];
    else if ($type == 2) $voucher_no = 'RV-' . $header['voucher_id'];
    else                 $voucher_no = 'CV-' . $header['voucher_id'];
    ...
    if ($type == 1)      $view = 'account/pos_print_payment_voucher';
    else if ($type == 2) $view = 'account/pos_print_receipt_voucher';
    else                 $view = 'account/pos_print_contra_voucher';

    $data['details'] = $this->load->view($view, $data, true);
