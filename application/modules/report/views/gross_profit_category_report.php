...
<script>
    ...
    function generateReport() {
        $.ajax({
            type: "post",
            url: $('#baseUrl2').val() + 'report/report/gross_profit_categorywise_report',
            data: {
                from_date: $('#from_date').val(),
                to_date: document.getElementById('single_date_checkbox').checked ?
                    $('#from_date').val() : $('#to_date').val(),
                empid: type, // ✅ NULL goes here
                istype: document.getElementById('single_date_checkbox').checked,
                category: $('#category').val() || null, // ✅ handle empty
                product: $('#productid').val() || null // ✅ handle empty
            },
            success: function(data1) {
                let datas = JSON.parse(data1);
                console.log(datas)
                if (datas && datas.length > 0) {
                    window.open(`generate_grossprofitreportcategorywise`, '_blank');
                } else {
