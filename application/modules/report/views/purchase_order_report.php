			...
			<div class="panel-body" style="margin-left: 120px;">
				...
				<div class="form-group">
					...
					<div class="input-group mr-4" style="width: 250px;">
						<select class="form-control" id="branch" required name="branch" style="width: 250px;" tabindex="3"></select>
					</div>
				</div>
				...
				<button type="button" id="btn-filter" class="btn btn-success" onclick="onPurchaseOrderFilterButtonClick()">
					Generate Report
				</button>
			</div>
		</div>
...
<script>
	...
	function onPurchaseOrderFilterButtonClick() {
		type = type2;
	...
	function generateReport() {
		$.ajax({
			type: "post",
			url: $('#baseUrl2').val() + 'report/report/purchase_order_reportinvoicewise',
			data: {
			...
			success: function(data1) {
				...
				if (datas.length > 0) {
					window.open('generate_purchaseorderreportinvoice', '_blank');
				} else {
