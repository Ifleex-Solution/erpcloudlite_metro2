...
");
    ...
    public function bdtask_new_stock_form($id = null)
    {
        ...
        if (!$this->permission1->method('new_inventory_transection', 'create')->access() && $this->session->userdata('user_level2') != 3) {
            $previous_url = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : base_url();
            redirect($previous_url);
        }
    ...
    public function bdtask_manage_inventory_transection()
    {
        ...
        if (!$this->permission1->method('manage_inventory_transection', 'read')->access() && $this->session->userdata('user_level2') != 3) {
            $previous_url = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : base_url();
            redirect($previous_url);
        }
        echo modules::run('template/layout', $data);
